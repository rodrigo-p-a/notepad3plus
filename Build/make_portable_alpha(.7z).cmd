...
rem * Notepad3                                                                   *
rem *                                                                            *
rem * make_portable_alpha(.7z).cmd                                               *
rem *   Batch file for creating "Portable alpha (*.7z)" packages                 *
rem *                                                                            *
rem * See License.txt for details about distribution and modification.           *
rem *                                                                            *
rem *                                                 (c) Rizonesoft 2008-2026   *
...
SET INPUTDIRx86="bin\Release_x86_v145"
SET INPUTDIRx64="bin\Release_x64_v145"
...
PUSHD "packages"
IF EXIST "Notepad3_%NP3_VER%.7z" DEL "Notepad3_%NP3_VER%.7z"
IF EXIST "%TEMP_NAME%"      RD /S /Q "%TEMP_NAME%"
IF NOT EXIST "%TEMP_NAME%"  MD "%TEMP_NAME%"

IF EXIST "Notepad3_%NP3_VER%*.7z" COPY /Y /V "Notepad3_%NP3_VER%*.7z" "%TEMP_NAME%\" >NUL
IF EXIST "%TEMP_NAME%\Notepad3_%NP3_VER%*.7z" DEL /F /Q "Notepad3_%NP3_VER%*.7z" >NUL

PUSHD "%TEMP_NAME%"

"%SEVENZIP%" a -t7z -ms=on -mx=7 Notepad3_%NP3_VER%.7z * >NUL
IF %ERRORLEVEL% NEQ 0 CALL :SUBMSG "ERROR" "Compilation failed!"

CALL :SUBMSG "INFO" "Notepad3_%NP3_VER%_Portable.7z created successfully!"

MOVE /Y "Notepad3_%NP3_VER%.7z" "..\Notepad3_%NP3_VER%_alpha_Portable.7z" >NUL
...
:SubZipFiles
SET "ZIP_NAME=Notepad3_%NP3_VER%_alpha_%2%_Portable"
TITLE Creating %ZIP_NAME%.7z...
CALL :SUBMSG "INFO" "Creating %ZIP_NAME%.7z..."
IF EXIST "%TEMP_NAME%"     RD /S /Q "%TEMP_NAME%"
IF NOT EXIST "%TEMP_NAME%" MD "%TEMP_NAME%"
...
MOVE /Y "%ZIP_NAME%.7z" "..\packages" >NUL
POPD
IF EXIST "%TEMP_NAME%" RD /S /Q "%TEMP_NAME%"
EXIT /B
