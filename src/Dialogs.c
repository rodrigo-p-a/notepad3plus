...
//  (e.g. minipath) is not present next to Notepad3.exe, extract the copy that
//  the build step embedded as an RCDATA resource into the per-user folder and
//  return its full path. Reuses an already-extracted copy of the same size.
//
static bool _ExtractEmbeddedExe(LPCWSTR resName, LPCWSTR fileName, HPATHL hOut)
{
    HRSRC const hRes = FindResourceW(Globals.hInstance, resName, RT_RCDATA);
    if (!hRes) {
        return false; // not embedded (e.g. dev build) -> caller handles fallback
    }
    ...
    Path_CreateDirectoryEx(hOut);
    Path_Append(hOut, fileName);

    if (Path_IsExistingFile(hOut)) {
        HANDLE const hChk = Path_CreateFile(hOut, GENERIC_READ, FILE_SHARE_READ, NULL,
    ...
    HANDLE const h = Path_CreateFile(hOut, GENERIC_WRITE, 0, NULL,
...
void DialogFileBrowse(HWND hwnd)
{
    ...
    if (Path_IsRelative(hExeFile)) {
        ...
        if (Path_IsExistingFile(hTemp)) {
            ...
            // single-exe distribution: extract the embedded helper and use it
            HPATHL hEmb = Path_Allocate(NULL);
            if (_ExtractEmbeddedExe(L"MINIPATH", L"minipath.exe", hEmb)) {
                Path_Swap(hExeFile, hEmb);
            }
            Path_Release(hEmb);
        }
        Path_Release(hTemp);
