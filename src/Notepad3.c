...
// Multi-tab session persistence (definitions live below, near the tab subsystem)
bool Tabs_SaveSession(void);
void Tabs_RestoreSession(void);
...
HWND InitInstance(const HINSTANCE hInstance, int nCmdShow)
{
    ...
    if (!bOpened) {
        ...
        // restore the previous session's open files as tabs (no file-arg start)
        if (!s_IsThisAnElevatedRelaunch) {
            Tabs_RestoreSession();
        }
    }
...
// Multi-document tab bar + split panes: forward decls (definitions live below)
bool Tabs_PromptSaveAllForClose(void);
bool Split_OnLButtonDown(HWND hwnd, POINT pt);
void Split_OnMouseMove(HWND hwnd, POINT pt);
void Split_OnLButtonUp(void);
bool Split_OnSetCursor(HWND hwnd);
...
LRESULT CALLBACK MainWndProc(HWND hwnd, UINT umsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_LBUTTONDOWN: {
        ...
        if (Split_OnLButtonDown(hwnd, pt)) {
            return 0;
        }
    } break;
    ...
    case WM_SETCURSOR:
        if (((HWND)wParam == hwnd) && Split_OnSetCursor(hwnd)) {
            return TRUE;
        }
        break;
    ...
    case WM_CLOSE:
        ...
        {
            // "remembers itself": back up the whole session (incl. unsaved &
            // untitled) to temp files; only prompt if that store isn't writable.
            bool const proceed = (Settings.SaveRecentFiles && Tabs_SaveSession()) || Tabs_PromptSaveAllForClose();
            if (proceed) {
    ...
    case WM_QUERYENDSESSION:
        ...
        else if ((Settings.SaveRecentFiles && Tabs_SaveSession()) || Tabs_PromptSaveAllForClose()) {
            return TRUE;
        }
        break;
...
//=============================================================================
//
//  Multi-document tab bar (Phase 1: one live Scintilla view, swapped docs)
//
//  Each tab owns a Scintilla document pointer (SCI_CREATEDOCUMENT) plus the
//  per-document state that Notepad3 keeps in globals (path, encoding, EOL,
//  file-vars, caret/scroll, bookmarks). Switching tabs saves the active
//  globals into the current EditBuffer and restores them from the target.
//
//=============================================================================

#define NP3_MAX_TABS 256

typedef struct EditBuffer {
    sptr_t    pDoc;              // Scintilla document pointer (owned reference)
    HPATHL    path;             // file path (empty => untitled)
    cpi_enc_t encoding;
    int       eolMode;
    bool      inconsistentEOLs;
    bool      needSaving;       // mirror of s_NeedSavingForced
    DocPos    caretPos;
    DocPos    anchorPos;
    DocLn     firstVisibleLine;
    FILEVARS  fv;
    HSTRINGW  bookmarks;
    WCHAR     titleExcerpt[MIDSZ_BUFFER];
    int       id;
    int       saveIdx;          // transient: flat index used for session layout (de)serialization
} EditBuffer;

//-----------------------------------------------------------------------------
//  Phase 2: each pane is a live Scintilla view with its own tab strip.
//  Panes are arranged in a binary split tree. The "active pane" owns the
//  global SciCall target (g_hwndEditWindow) via InitScintillaHandle().
//-----------------------------------------------------------------------------

#define NP3_MAX_PANES 16
#define NP3_SPLIT_BAR 6      // splitter thickness (px)

typedef struct EditPane {
    HWND        hwndSci;                 // live Scintilla view window
    HWND        hwndTabBar;              // this pane's tab strip
    EditBuffer* tabs[NP3_MAX_TABS];
    int         tabCount;
    int         tabActive;
    int         cyTabBar;
    RECT        rc;                      // last laid-out content rect
} EditPane;

typedef enum { SPL_LEAF = 0, SPL_HORZ, SPL_VERT } SplitKind; // HORZ: top/bottom, VERT: left/right
typedef struct SplitNode {
    SplitKind         kind;
    EditPane*         pane;              // leaf only
    struct SplitNode* child[2];
    struct SplitNode* parent;
    float             ratio;            // child[0] fraction
    RECT              bar;              // splitter hit-rect (internal nodes)
} SplitNode;

static EditPane   s_panes[NP3_MAX_PANES];
static int        s_paneCount    = 0;
static EditPane*  g_activePane   = &s_panes[0];   // never NULL after startup
static SplitNode* g_splitRoot    = NULL;
static int        s_tabNextId    = 1;
static bool       s_tabSwitching = false;         // re-entrancy guard for TCN_SELCHANGE
static SplitNode* s_dragBar      = NULL;          // splitter currently being dragged

// active-pane aliases: the Phase-1 tab code now operates on g_activePane
#define s_hwndTabBar  (g_activePane->hwndTabBar)
#define s_tabs        (g_activePane->tabs)
#define s_tabCount    (g_activePane->tabCount)
#define s_tabActive   (g_activePane->tabActive)
#define s_cyTabBar    (g_activePane->cyTabBar)

// forward decls used below
bool        Tabs_IsActive(void);
int         Tabs_Count(void);
void        Tabs_SyncActiveFromGlobals(void);
bool        FileLoadTab(const HPATHL hfile_pth, FileLoadFlags fLoadFlags, const DocPos curPos, const DocLn visLn);
void        SetActivePane(EditPane* pane);
static void SplitLayout_Resize(SplitNode* node, RECT rc);
void        Pane_DoSplit(SplitKind kind);
void        Pane_CloseActive(void);
static SplitNode* _Split_NewLeaf(EditPane* pane);
static LRESULT CALLBACK _TabBarSubclassProc(HWND, UINT, WPARAM, LPARAM, UINT_PTR, DWORD_PTR);
bool        Tabs_CloseAt(int idx);
void        Tabs_Activate(int idx);

//=============================================================================
//
//  _Tabs_BuildLabel() - compose the visible tab caption for a buffer
//
static void _Tabs_BuildLabel(const EditBuffer* buf, LPWSTR out, int cch, bool bModified)
{
    WCHAR name[MAX_PATH_EXPLICIT] = { L'\0' };
    if (Path_IsNotEmpty(buf->path)) {
        StringCchCopyW(name, COUNTOF(name), Path_FindFileName(buf->path));
    } else if (StrIsNotEmpty(buf->titleExcerpt)) {
        StringCchCopyW(name, COUNTOF(name), buf->titleExcerpt);
    } else {
        GetLngString(IDS_MUI_UNTITLED, name, COUNTOF(name));
    }
    StringCchPrintfW(out, cch, L"%s%s", bModified ? L"● " : L"  ", name);
}

//=============================================================================
//
//  _Tabs_UpdateItemText() - refresh one tab's caption in the control
//
static void _Tabs_UpdateItemText(int idx)
{
    if ((idx < 0) || (idx >= s_tabCount) || !s_hwndTabBar) {
        return;
    }
    bool const bModified = (idx == s_tabActive) ? IsSaveNeeded() : s_tabs[idx]->needSaving;
    WCHAR label[MAX_PATH_EXPLICIT + 8] = { L'\0' };
    _Tabs_BuildLabel(s_tabs[idx], label, COUNTOF(label), bModified);
    TCITEM tci = { 0 };
    tci.mask = TCIF_TEXT;
    tci.pszText = label;
    TabCtrl_SetItem(s_hwndTabBar, idx, &tci);
}

//=============================================================================
//
//  _Tabs_SaveActiveState() - capture the live globals into the active buffer
//
static void _Tabs_SaveActiveState(void)
{
    if ((s_tabActive < 0) || (s_tabActive >= s_tabCount)) {
        return;
    }
    EditBuffer* const buf = s_tabs[s_tabActive];
    buf->pDoc             = SciCall_GetDocPointer();
    buf->caretPos         = SciCall_GetCurrentPos();
    buf->anchorPos        = SciCall_GetAnchor();
    buf->firstVisibleLine = SciCall_GetFirstVisibleLine();
    buf->encoding         = Encoding_GetCurrent();
    buf->eolMode          = SciCall_GetEOLMode();
    buf->inconsistentEOLs = Globals.bDocHasInconsistentEOLs;
    buf->needSaving       = IsSaveNeeded();
    buf->fv               = Globals.fvCurFile;
    Path_Reset(buf->path, Path_Get(Paths.CurrentFile));
    StringCchCopyW(buf->titleExcerpt, COUNTOF(buf->titleExcerpt), s_wchTitleExcerpt);
    WCHAR wchBookMarks[MRU_BMRK_SIZE] = { L'\0' };
    EditGetBookmarkList(Globals.hwndEdit, wchBookMarks, COUNTOF(wchBookMarks));
    StrgReset(buf->bookmarks, wchBookMarks);
}

//=============================================================================
//
//  _Tabs_LoadActiveState() - make buffer[idx] the live document + globals
//
static void _Tabs_LoadActiveState(int idx)
{
    if ((idx < 0) || (idx >= s_tabCount)) {
        return;
    }
    EditBuffer* const buf = s_tabs[idx];
    s_tabActive = idx;

    SciCall_SetDocPointer(buf->pDoc);

    Path_Reset(Paths.CurrentFile, Path_Get(buf->path));
    Globals.fvCurFile = buf->fv;
    Encoding_Current(buf->encoding);
    SciCall_SetEOLMode(buf->eolMode);
    Globals.bDocHasInconsistentEOLs = buf->inconsistentEOLs;
    s_NeedSavingForced = buf->needSaving;
    StringCchCopyW(s_wchTitleExcerpt, COUNTOF(s_wchTitleExcerpt), buf->titleExcerpt);

    // re-apply lexer/styling for the swapped document
    if (Path_IsNotEmpty(buf->path)) {
        Style_SetLexerFromFile(Globals.hwndEdit, Paths.CurrentFile);
    } else {
    ...
    LPCWSTR const pszBM = StrgGet(buf->bookmarks);
    EditSetBookmarkList(Globals.hwndEdit, pszBM ? pszBM : L"");

    // restore caret / selection / scroll
    if ((buf->caretPos >= 0) && (buf->anchorPos >= 0)) {
        Sci_SetStreamSelection(buf->anchorPos, buf->caretPos, true);
    }
    SciCall_SetFirstVisibleLine(buf->firstVisibleLine);

    _SetEnumWindowsItems(Globals.hwndMain);

    // re-target file watching to the now-active file
    AutoSaveStop();
    InstallFileWatching(false);
    InstallFileWatching(true);
...
//=============================================================================
//
//  Tabs_Activate() - switch the live view to the tab at index
//
void Tabs_Activate(int idx)
{
    if ((idx < 0) || (idx >= s_tabCount) || (idx == s_tabActive)) {
        if (idx == s_tabActive) {
            SetFocus(Globals.hwndEdit);
        }
        return;
    }
    _Tabs_SaveActiveState();
    _Tabs_UpdateItemText(s_tabActive); // refresh old tab's modified mark
    _Tabs_LoadActiveState(idx);
    _Tabs_UpdateItemText(idx);

    if (s_hwndTabBar && (TabCtrl_GetCurSel(s_hwndTabBar) != idx)) {
        s_tabSwitching = true;
        TabCtrl_SetCurSel(s_hwndTabBar, idx);
        s_tabSwitching = false;
    }
    SetFocus(Globals.hwndEdit);
}
...
//=============================================================================
//
//  _Tabs_NewBuffer() - allocate an EditBuffer wrapping a fresh document
//
static EditBuffer* _Tabs_NewBuffer(sptr_t pDoc)
{
    EditBuffer* const buf = (EditBuffer*)AllocMem(sizeof(EditBuffer), HEAP_ZERO_MEMORY);
    if (!buf) {
        return NULL;
    }
    buf->pDoc             = pDoc;
    buf->path             = Path_Allocate(L"");
    buf->bookmarks        = StrgCreate(L"");
    buf->encoding         = Settings.DefaultEncoding;
    buf->eolMode          = Settings.DefaultEOLMode;
    buf->caretPos         = 0;
    buf->anchorPos        = 0;
    buf->firstVisibleLine = 0;
    buf->id               = s_tabNextId++;
    FileVars_GetFromData(NULL, 0, &buf->fv); // zero-init file vars
    return buf;
}

//=============================================================================
//
//  _Tabs_InsertItem() - add a tab caption to the control
//
static void _Tabs_InsertItem(int idx, const EditBuffer* buf)
{
    if (!s_hwndTabBar) {
        return;
    }
    WCHAR label[MAX_PATH_EXPLICIT + 8] = { L'\0' };
    _Tabs_BuildLabel(buf, label, COUNTOF(label), false);
    TCITEM tci = { 0 };
    tci.mask = TCIF_TEXT;
    tci.pszText = label;
    TabCtrl_InsertItem(s_hwndTabBar, idx, &tci);
}

//=============================================================================
//
//  Tabs_AddAndActivate() - create a new untitled tab, switch to it
//
int Tabs_AddAndActivate(void)
{
    if (s_tabCount >= NP3_MAX_TABS) {
        return s_tabActive;
    }
    // persist the document currently shown before creating a new one
    _Tabs_SaveActiveState();
    if (s_tabActive >= 0) {
        _Tabs_UpdateItemText(s_tabActive);
    }

    sptr_t const pDoc = SciCall_CreateDocument(0, SC_DOCUMENTOPTION_DEFAULT);
    EditBuffer* const buf = _Tabs_NewBuffer(pDoc);
    if (!buf) {
        return s_tabActive;
    }
    int const idx = s_tabCount;
    s_tabs[idx] = buf;
    s_tabCount++;
    _Tabs_InsertItem(idx, buf);
...
//=============================================================================
//
//  _Tabs_FreeBuffer() - release a buffer's document + memory (not shown)
//
static void _Tabs_FreeBuffer(EditBuffer* buf)
{
    if (!buf) {
        return;
    }
    if (buf->pDoc) {
        SciCall_ReleaseDocument(buf->pDoc);
    }
    Path_Release(buf->path);
    StrgDestroy(buf->bookmarks);
    FreeMem(buf);
}

//=============================================================================
//
//  _Tabs_RemoveAt() - drop tab[idx] from arrays + control (no save prompt).
//  Assumes the view is NOT currently showing buf[idx].
//
static void _Tabs_RemoveAt(int idx)
{
    if ((idx < 0) || (idx >= s_tabCount)) {
        return;
    }
    _Tabs_FreeBuffer(s_tabs[idx]);
    for (int i = idx; i < s_tabCount - 1; ++i) {
        s_tabs[i] = s_tabs[i + 1];
    }
    s_tabs[--s_tabCount] = NULL;
    if (s_hwndTabBar) {
        TabCtrl_DeleteItem(s_hwndTabBar, idx);
    }
    if (s_tabActive > idx) {
        s_tabActive--;
    }
...
//=============================================================================
//
//  Tabs_ActiveIsReusable() - true if the active tab is a pristine untitled doc
//
bool Tabs_ActiveIsReusable(void)
{
    return (s_tabCount > 0) && Path_IsEmpty(Paths.CurrentFile) &&
           (SciCall_GetTextLength() <= 0) && !IsSaveNeeded();
}
...
bool Tabs_CloseAt(int idx)
{
    ...
    if (s_tabCount <= 1) {
        ...
        FileLoad(hempty, FLF_New | FLF_DontSave, 0, 0);
        Path_Release(hempty);
        Tabs_SyncActiveFromGlobals();
        return true;
    }

    int const neighbor = (idx + 1 < s_tabCount) ? (idx + 1) : (idx - 1);
    Tabs_Activate(neighbor);   // show a surviving doc before releasing this one
    _Tabs_RemoveAt(idx);       // s_tabActive adjusted inside if needed

    // keep tab control selection in sync with s_tabActive
    if (s_hwndTabBar) {
        s_tabSwitching = true;
        TabCtrl_SetCurSel(s_hwndTabBar, s_tabActive);
        s_tabSwitching = false;
    }
    SetFocus(Globals.hwndEdit);
    return true;
...
//=============================================================================
//
//  _Tabs_DiscardOrphanActive() - silently drop the active (failed-load) tab
//
static void _Tabs_DiscardOrphanActive(void)
{
    if (s_tabCount <= 1) {
        return;
    }
    int const idx = s_tabActive;
    int const neighbor = (idx + 1 < s_tabCount) ? (idx + 1) : (idx - 1);
    Tabs_Activate(neighbor);
    _Tabs_RemoveAt(idx);
    if (s_hwndTabBar) {
        s_tabSwitching = true;
        TabCtrl_SetCurSel(s_hwndTabBar, s_tabActive);
...
//=============================================================================
//
//  Tabs_SyncActiveFromGlobals() - refresh active buffer mirror + caption
//  Call after FileLoad / FileSave changed Paths.CurrentFile / modified state.
//
void Tabs_SyncActiveFromGlobals(void)
{
    if ((s_tabActive < 0) || (s_tabActive >= s_tabCount)) {
...
//=============================================================================
//
//  Tabs_Next() / Tabs_Prev() - cycle the active tab
//
void Tabs_Next(void)
{
    if (s_tabCount > 1) {
        Tabs_Activate((s_tabActive + 1) % s_tabCount);
    }
}
void Tabs_Prev(void)
{
    if (s_tabCount > 1) {
        Tabs_Activate((s_tabActive + s_tabCount - 1) % s_tabCount);
    }
}

//=============================================================================
//
//  Tabs_PromptSaveAllForClose() - on app exit, prompt to save every dirty tab.
//  Returns false if the user cancelled (abort the close).
//
bool Tabs_PromptSaveAllForClose(void)
{
    if (s_tabCount <= 0) {
        return FileSave(FSF_Ask);
    }
...
int  Tabs_Count(void)        { return s_tabCount; }
bool Tabs_IsActive(void)     { return (s_tabCount > 0); }
int  Tabs_ActiveIndex(void)  { return s_tabActive; }
int  Tabs_BarHeight(void)    { return s_cyTabBar; }

//=============================================================================
//
//  Tabs_CreateBar() - create the tab strip + wrap the initial document
//
static void Tabs_CreateBar(HWND hwndParent, HINSTANCE hInstance)
{
    s_hwndTabBar = CreateWindowEx(
        0, WC_TABCONTROL, NULL,
        WS_CHILD | WS_CLIPSIBLINGS | TCS_FOCUSNEVER | TCS_TOOLTIPS | TCS_SINGLELINE,
        0, 0, 0, 0,
        hwndParent, (HMENU)IDC_TABBAR, hInstance, NULL);

    if (!s_hwndTabBar) {
        return;
    }
    SendMessage(s_hwndTabBar, WM_SETFONT, (WPARAM)GetStockObject(DEFAULT_GUI_FONT), MAKELPARAM(TRUE, 0));
    SetWindowSubclass(s_hwndTabBar, _TabBarSubclassProc, 0, 0);

#ifdef D_NP3_WIN10_DARK_MODE
    if (IsDarkModeSupported() && CheckDarkModeEnabled()) {
        AllowDarkModeForWindowEx(s_hwndTabBar, true);
        SetWindowTheme(s_hwndTabBar, L"DarkMode_Explorer", NULL);
    }
#endif

    // measure tab strip height from the control's item rect
    TCITEM tci = { 0 };
    tci.mask = TCIF_TEXT;
    tci.pszText = L"Tab";
    TabCtrl_InsertItem(s_hwndTabBar, 0, &tci);
    RECT rcItem = { 0 };
    if (TabCtrl_GetItemRect(s_hwndTabBar, 0, &rcItem)) {
        s_cyTabBar = (rcItem.bottom - rcItem.top) + 6;
    } else {
        s_cyTabBar = 26;
    }
    TabCtrl_DeleteItem(s_hwndTabBar, 0);

    // the root pane wraps the existing Scintilla view (Globals.hwndEdit)
    g_activePane->hwndSci = Globals.hwndEdit;

    // wrap the document Scintilla created with the view as buffer 0
    sptr_t const pDoc0 = SciCall_GetDocPointer();
    SciCall_AddRefDocument(pDoc0); // buffer now owns a reference too
    EditBuffer* const buf0 = _Tabs_NewBuffer(pDoc0);
    s_tabs[0] = buf0;
    s_tabCount = 1;
    s_tabActive = 0;
    _Tabs_InsertItem(0, buf0);
    TabCtrl_SetCurSel(s_hwndTabBar, 0);
    ShowWindow(s_hwndTabBar, SW_SHOW);

    // establish the split tree (single root leaf) + hide the legacy border frame
    s_paneCount = 1;
    g_splitRoot = _Split_NewLeaf(g_activePane);
    ShowWindow(s_hwndEditFrame, SW_HIDE);
}

//=============================================================================
//
//  FileLoadTab() - open a file in a tab (reuse a pristine tab or add a new
//  one), delegating the actual load to FileLoad().
//
bool FileLoadTab(const HPATHL hfile_pth, FileLoadFlags fLoadFlags, const DocPos curPos, const DocLn visLn)
{
    bool createdNew = false;
    if (s_tabCount > 0) {
        if (!Tabs_ActiveIsReusable()) {
            Tabs_AddAndActivate();
            createdNew = true;
        }
        fLoadFlags |= FLF_DontSave; // target tab is blank => never prompt previous tab
    }
    bool const ok = FileLoad(hfile_pth, fLoadFlags, curPos, visLn);
    if (s_tabCount > 0) {
        if (!ok && createdNew && Path_IsEmpty(Paths.CurrentFile) && (SciCall_GetTextLength() <= 0)) {
            _Tabs_DiscardOrphanActive();
        } else {
            Tabs_SyncActiveFromGlobals();
        }
    }
...
//=============================================================================
//
//  Multi-tab session persistence (Phase 3) - "notepad that remembers itself".
//  On close, every editor (incl. untitled / unsaved / empty) is backed up to a
//  temp file beside the exe; on next start everything is reopened verbatim.
//  Gated on Settings.SaveRecentFiles. The split-tree layout is not persisted;
//  tabs are restored flat into the (single) startup pane.
//
#define INI_SECTION_TABS L"Session"

static bool s_sessionSaved = false; // guard: back up only once per shutdown

//  per-user session folder: %LOCALAPPDATA%\Rizonesoft\Notepad3\Session
//  (in the user profile, NOT beside the exe -> works from read-only/Program Files
//   installs and survives moving the binary).
static bool _Session_BaseDir(HPATHL hOut, bool create)
{
    if (!Path_GetKnownFolder(&FOLDERID_LocalAppData, hOut)) {
        return false;
    }
    Path_Append(hOut, L"Rizonesoft");
    Path_Append(hOut, L"Notepad3");
    Path_Append(hOut, L"Session");
    if (create) {
        HRESULT const hr = Path_CreateDirectoryEx(hOut);
        return SUCCEEDED(hr) || (hr == HRESULT_FROM_WIN32(ERROR_ALREADY_EXISTS));
    }
    return Path_IsExistingDirectory(hOut);
}

//  path of the self-contained session index file: <SessionDir>\session.ini
//  (decoupled from the main INI so "remember" works even without a Notepad3.ini)
static bool _Session_IniPath(bool create, HPATHL hOut)
{
    HPATHL hdir = Path_Allocate(NULL);
    bool const ok = _Session_BaseDir(hdir, create);
    Path_Reset(hOut, Path_Get(hdir));
    Path_Append(hOut, L"session.ini");
    Path_Release(hdir);
    return ok;
}

//  path of a per-tab backup blob: <SessionDir>\<name>
static HPATHL _Session_BakPath(const WCHAR* name)
{
    HPATHL h = Path_Allocate(NULL);
    _Session_BaseDir(h, false);
    Path_Append(h, name);
    return h;
}

//  make the index file UTF-16LE (BOM) so WritePrivateProfileStringW stores
//  unicode paths correctly
static void _Session_EnsureBom(const HPATHL hIni)
{
    if (Path_IsExistingFile(hIni)) {
        return;
    }
    HANDLE const h = Path_CreateFile(hIni, GENERIC_WRITE, FILE_SHARE_READ, NULL,
...
//  write a document's UTF-8 text to a backup file (uses the active view as a
//  scratch surface via SetDocPointer; caller restores the original doc).
static void _Session_BackupDoc(sptr_t pDoc, const HPATHL hbak)
{
    SciCall_SetDocPointer(pDoc);
    DocPos const len = SciCall_GetTextLength();
    char* const buf = (char*)AllocMem((size_t)len + 1, HEAP_ZERO_MEMORY);
    if (!buf) {
        return;
    }
    SciCall_GetText(len + 1, buf);
    HANDLE const h = Path_CreateFile(hbak, GENERIC_WRITE, FILE_SHARE_READ, NULL,
                                     CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    if (h != INVALID_HANDLE_VALUE) {
        DWORD written = 0;
...
//  read raw bytes of a backup file
static bool _Session_ReadFile(const HPATHL hbak, char** outBuf, DocPosU* outLen)
{
    *outBuf = NULL;
    *outLen = 0;
    HANDLE const h = Path_CreateFile(hbak, GENERIC_READ, FILE_SHARE_READ, NULL,
    ...
    char* const buf = (char*)AllocMem((size_t)sz.QuadPart + 1, HEAP_ZERO_MEMORY);
    if (!buf) {
        CloseHandle(h);
        return false;
    }
...
//  session restore working set (file-scope so the layout realizer can use it)
static WCHAR       s_rsFiles[NP3_MAX_TABS][MAX_PATH_EXPLICIT];
static WCHAR       s_rsBaks[NP3_MAX_TABS][40];
static WCHAR       s_rsNames[NP3_MAX_TABS][MIDSZ_BUFFER];
static int         s_rsMods[NP3_MAX_TABS];
static EditBuffer* s_rsBufByFlat[NP3_MAX_TABS + 1]; // 1-based flat index -> restored buffer
static int         s_rsCount = 0;
static bool        _Layout_Realize(LPCWSTR layout); // defined after the split machinery

//  during restore each entry gets its own tab; only the very first reuses the
//  pristine startup tab (otherwise consecutive blank tabs would collapse).
static bool s_restoreFirstPending = false;
static void _RestoreEnsureTab(void)
{
    if (s_restoreFirstPending && Tabs_IsActive() && Tabs_ActiveIsReusable()) {
        s_restoreFirstPending = false; // reuse the startup blank once
        return;
    }
    s_restoreFirstPending = false;
    if (Tabs_IsActive()) {
        Tabs_AddAndActivate();
    }
}

//  restore one tab from backed-up content
static void _RestoreBackupTab(LPCWSTR origPath, const char* content, DocPosU len, bool modified, LPCWSTR name)
{
    _RestoreEnsureTab();
    EditSetNewText(Globals.hwndEdit, content ? content : "", len, true, false);
    if (StrIsNotEmpty(origPath)) {
        Path_Reset(Paths.CurrentFile, origPath);
        Style_SetLexerFromFile(Globals.hwndEdit, Paths.CurrentFile);
    } else {
        Path_Empty(Paths.CurrentFile, false);
        Style_SetDefaultLexer(Globals.hwndEdit);
    }
    StringCchCopyW(s_wchTitleExcerpt, COUNTOF(s_wchTitleExcerpt), name ? name : L"");
    if (modified) {
        SetSaveNeeded(true);
    } else {
        SetSavePoint();
...
//  restore an empty untitled tab (optionally with a custom display name)
static void _RestoreBlankTab(LPCWSTR name)
{
    _RestoreEnsureTab();
    StringCchCopyW(s_wchTitleExcerpt, COUNTOF(s_wchTitleExcerpt), name ? name : L"");
    Tabs_SyncActiveFromGlobals();
...
//  restore working-set entry i (0-based) into the active pane; tags the created
//  buffer with its flat index (i+1) so the layout realizer can place it.
static void _RestoreOneByFlat(int i)
{
    if ((i < 0) || (i >= s_rsCount)) {
        return;
    }
    bool const hasBak  = StrIsNotEmpty(s_rsBaks[i]);
    bool const hasFile = StrIsNotEmpty(s_rsFiles[i]);
    bool done = false;
    if (hasBak) {
        HPATHL hbak = _Session_BakPath(s_rsBaks[i]);
        char* content = NULL;
        DocPosU clen = 0;
        if (_Session_ReadFile(hbak, &content, &clen)) {
            _RestoreBackupTab(s_rsFiles[i], content, clen, s_rsMods[i] != 0, s_rsNames[i]);
            FreeMem(content);
            done = true;
        }
        Path_Release(hbak);
    ...
    if (!done && hasFile) {
        HPATHL hp = Path_Allocate(s_rsFiles[i]);
        if (Path_IsExistingFile(hp)) {
            _RestoreEnsureTab();
            FileLoad(hp, FLF_DontSave, 0, 0); // let encoding/EOL auto-detect run
            Tabs_SyncActiveFromGlobals();
            done = true;
        }
        Path_Release(hp);
    ...
    if (!done) {
        _RestoreBlankTab(s_rsNames[i]);
    }
    EditBuffer* const b = g_activePane->tabs[g_activePane->tabActive];
    b->saveIdx = i + 1;
    s_rsBufByFlat[i + 1] = b;
}

//  serialize the split tree to a compact string, e.g. V50(L1:1,2)(H50(L3:3)(L4:4))
static void _Layout_Serialize(SplitNode* node, WCHAR* out, int cch)
{
    if (!node || (cch <= 0)) {
        return;
    }
    WCHAR seg[64];
    if (node->kind == SPL_LEAF) {
        EditPane* const p = node->pane;
        if (!p) {
            return;
        }
        int const activeFlat = ((p->tabActive >= 0) && (p->tabActive < p->tabCount)) ? p->tabs[p->tabActive]->saveIdx : 0;
        StringCchPrintfW(seg, COUNTOF(seg), L"L%d:", activeFlat);
        StringCchCatW(out, cch, seg);
        for (int t = 0; t < p->tabCount; ++t) {
            StringCchPrintfW(seg, COUNTOF(seg), (t == 0) ? L"%d" : L",%d", p->tabs[t]->saveIdx);
            StringCchCatW(out, cch, seg);
        }
    } else {
        StringCchPrintfW(seg, COUNTOF(seg), L"%c%d(", (node->kind == SPL_VERT) ? L'V' : L'H',
                         (int)(node->ratio * 100.0f + 0.5f));
        StringCchCatW(out, cch, seg);
        _Layout_Serialize(node->child[0], out, cch);
        StringCchCatW(out, cch, L")(");
        _Layout_Serialize(node->child[1], out, cch);
        StringCchCatW(out, cch, L")");
    }
}

//  back up every tab (content + metadata). Returns true on success; false if
//  the session store isn't writable (caller then falls back to a save prompt).
bool Tabs_SaveSession(void)
{
    if (s_sessionSaved) {
        return true;
    ...
    if (!Settings.SaveRecentFiles) {
        return false; // feature off => caller prompts to save
    }
    _Tabs_SaveActiveState(); // flush live globals into the active buffer
    HPATHL hIni = Path_Allocate(NULL);
    if (!_Session_IniPath(true, hIni)) {
        Path_Release(hIni);
        return false; // store not writable
    }
    _Session_EnsureBom(hIni);
    LPCWSTR const ini = Path_Get(hIni);

    sptr_t const origDoc = SciCall_GetDocPointer();
    int n = 0;
    int activeFlat = 1;
    for (int pi = 0; pi < NP3_MAX_PANES; ++pi) {
        EditPane* const p = &s_panes[pi];
        if (!p->hwndSci) {
            continue;
        }
        for (int t = 0; t < p->tabCount; ++t) {
            EditBuffer* const b = p->tabs[t];
            ++n;
            b->saveIdx = n; // tag for layout serialization
            bool const isActive = (p == g_activePane) && (t == p->tabActive);
            if (isActive) {
                activeFlat = n;
            }
            bool const named    = Path_IsNotEmpty(b->path);
            bool const modified = isActive ? IsSaveNeeded() : b->needSaving;
            WCHAR keyF[24], keyB[24], keyM[24], keyN[24], val[16];
            StringCchPrintfW(keyF, COUNTOF(keyF), L"File%d", n);
            StringCchPrintfW(keyB, COUNTOF(keyB), L"Backup%d", n);
            StringCchPrintfW(keyM, COUNTOF(keyM), L"Mod%d", n);
            StringCchPrintfW(keyN, COUNTOF(keyN), L"Name%d", n);
            WritePrivateProfileStringW(INI_SECTION_TABS, keyN, b->titleExcerpt, ini);
            WritePrivateProfileStringW(INI_SECTION_TABS, keyF, named ? Path_Get(b->path) : L"", ini);
            if (!named || modified) {
                WCHAR bakName[32];
                StringCchPrintfW(bakName, COUNTOF(bakName), L"tab%d.bak", n);
                HPATHL hbak = _Session_BakPath(bakName);
                _Session_BackupDoc(b->pDoc, hbak);
                Path_Release(hbak);
                WritePrivateProfileStringW(INI_SECTION_TABS, keyB, bakName, ini);
            } else {
                WritePrivateProfileStringW(INI_SECTION_TABS, keyB, L"", ini);
            }
            StringCchPrintfW(val, COUNTOF(val), L"%d", modified ? 1 : 0);
            WritePrivateProfileStringW(INI_SECTION_TABS, keyM, val, ini);
        }
    }
    SciCall_SetDocPointer(origDoc); // restore the active view's document

    // persist the split-pane layout (which tab lives in which pane, ratios)
    WCHAR layout[4096] = { L'\0' };
    _Layout_Serialize(g_splitRoot, layout, COUNTOF(layout));
    WritePrivateProfileStringW(INI_SECTION_TABS, L"Layout", layout, ini);

    WCHAR num[16];
    StringCchPrintfW(num, COUNTOF(num), L"%d", n);
    WritePrivateProfileStringW(INI_SECTION_TABS, L"Count", num, ini);
    StringCchPrintfW(num, COUNTOF(num), L"%d", activeFlat);
    WritePrivateProfileStringW(INI_SECTION_TABS, L"Active", num, ini);
    WritePrivateProfileStringW(NULL, NULL, NULL, ini); // flush
...
void Tabs_RestoreSession(void)
{
    ...
    HPATHL hIni = Path_Allocate(NULL);
    if (!_Session_IniPath(false, hIni) || !Path_IsExistingFile(hIni)) {
        Path_Release(hIni);
        return;
    }
    LPCWSTR const ini = Path_Get(hIni);

    int n = (int)GetPrivateProfileIntW(INI_SECTION_TABS, L"Count", 0, ini);
    if (n > NP3_MAX_TABS) {
        n = NP3_MAX_TABS;
    }
    int const activeFlat = (int)GetPrivateProfileIntW(INI_SECTION_TABS, L"Active", 1, ini);

    ZeroMemory(s_rsBufByFlat, sizeof(s_rsBufByFlat));
    s_rsCount = 0;
    for (int i = 1; i <= n; ++i) {
        WCHAR keyF[24], keyB[24], keyM[24], keyN[24];
        StringCchPrintfW(keyF, COUNTOF(keyF), L"File%d", i);
        StringCchPrintfW(keyB, COUNTOF(keyB), L"Backup%d", i);
        StringCchPrintfW(keyM, COUNTOF(keyM), L"Mod%d", i);
        StringCchPrintfW(keyN, COUNTOF(keyN), L"Name%d", i);
        GetPrivateProfileStringW(INI_SECTION_TABS, keyF, L"", s_rsFiles[s_rsCount], MAX_PATH_EXPLICIT, ini);
        GetPrivateProfileStringW(INI_SECTION_TABS, keyB, L"", s_rsBaks[s_rsCount], COUNTOF(s_rsBaks[s_rsCount]), ini);
        GetPrivateProfileStringW(INI_SECTION_TABS, keyN, L"", s_rsNames[s_rsCount], MIDSZ_BUFFER, ini);
        s_rsMods[s_rsCount] = (int)GetPrivateProfileIntW(INI_SECTION_TABS, keyM, 0, ini);
        ++s_rsCount;
    }

    static WCHAR s_layout[4096];
    GetPrivateProfileStringW(INI_SECTION_TABS, L"Layout", L"", s_layout, COUNTOF(s_layout), ini);
    Path_Release(hIni);

    bool laidOut = false;
    if (StrIsNotEmpty(s_layout)) {
        laidOut = _Layout_Realize(s_layout); // rebuilds panes + distributes tabs
    }

    if (!laidOut) {
        // flat fallback: all tabs in the startup pane
        s_restoreFirstPending = true;
        for (int i = 0; i < s_rsCount; ++i) {
            _RestoreOneByFlat(i);
        }
        if ((s_rsCount > 0) && (activeFlat >= 1) && (activeFlat <= Tabs_Count())) {
            Tabs_Activate(activeFlat - 1);
        }
    }
    else {
        // activate the pane/tab that owns the saved active flat index
        if ((activeFlat >= 1) && (activeFlat <= NP3_MAX_TABS) && s_rsBufByFlat[activeFlat]) {
            EditBuffer* const wb = s_rsBufByFlat[activeFlat];
            for (int pi = 0; pi < NP3_MAX_PANES; ++pi) {
                EditPane* const p = &s_panes[pi];
                if (!p->hwndSci) {
                    continue;
...
//=============================================================================
//
//  Split-pane management (Phase 2)
//
//=============================================================================

//  find a pane by its Scintilla view / tab-bar window
static EditPane* _Pane_FindBySci(HWND hwndSci)
{
    for (int i = 0; i < NP3_MAX_PANES; ++i) {
        if (s_panes[i].hwndSci == hwndSci && hwndSci) {
            return &s_panes[i];
        }
    }
    return NULL;
}
static EditPane* _Pane_FindByTabBar(HWND hwndBar)
{
    for (int i = 0; i < NP3_MAX_PANES; ++i) {
        if (s_panes[i].hwndTabBar == hwndBar && hwndBar) {
            return &s_panes[i];
        }
    }
    return NULL;
...
//  set per-document globals from a pane's active buffer WITHOUT touching the
//  document pointer (the view already displays it). Used on pane focus switch.
static void _Pane_SyncGlobalsFromActiveBuffer(EditPane* pane)
{
    if (!pane || (pane->tabActive < 0) || (pane->tabActive >= pane->tabCount)) {
        return;
    }
    EditBuffer* const buf = pane->tabs[pane->tabActive];
    Path_Reset(Paths.CurrentFile, Path_Get(buf->path));
    Globals.fvCurFile = buf->fv;
    Encoding_Current(buf->encoding);
    Globals.bDocHasInconsistentEOLs = buf->inconsistentEOLs;
...
//=============================================================================
//
//  SetActivePane() - make a pane the active SciCall target + sync UI/globals
//
void SetActivePane(EditPane* pane)
{
    if (!pane) {
    ...
    if (pane == g_activePane) {
        // already active, but SciCall may be stale (e.g. right after a split
        // left the global handle pointing at a freshly-created pane) - re-aim it
        if (g_hwndEditWindow != pane->hwndSci) {
            InitScintillaHandle(pane->hwndSci);
            Globals.hwndEdit = pane->hwndSci;
        }
        return;
    }
    // persist the currently active pane's live state into its active buffer
    _Tabs_SaveActiveState();
    if (g_activePane->tabActive >= 0) {
        _Tabs_UpdateItemText(g_activePane->tabActive);
    }
    // switch target
    g_activePane = pane;
    InitScintillaHandle(pane->hwndSci);
    Globals.hwndEdit = pane->hwndSci;
    _Pane_SyncGlobalsFromActiveBuffer(pane);
...
//=============================================================================
//
//  _Pane_Create() - allocate a pane: a fresh Scintilla view + tab bar + buffer.
//  Leaves the global SciCall handle targeting the new view.
//
static EditPane* _Pane_Create(void)
{
    int slot = -1;
    for (int i = 0; i < NP3_MAX_PANES; ++i) {
        if (s_panes[i].hwndSci == NULL) {
            slot = i;
            break;
        }
    }
    ...
    EditPane* const pane = &s_panes[slot];
    ZeroMemory(pane, sizeof(EditPane));
    pane->tabActive = -1;

    pane->hwndSci = CreateWindowEx(
        WS_EX_CLIENTEDGE, L"Scintilla", NULL,
        (WS_CHILD | WS_CLIPSIBLINGS) & ~WS_VISIBLE,
        0, 0, 0, 0, Globals.hwndMain, (HMENU)IDC_EDIT, Globals.hInstance, NULL);
    if (!pane->hwndSci) {
        return NULL;
    }
    InitScintillaHandle(pane->hwndSci);
    _InitializeSciEditCtrl(pane->hwndSci);

    pane->hwndTabBar = CreateWindowEx(
        0, WC_TABCONTROL, NULL,
        WS_CHILD | WS_CLIPSIBLINGS | TCS_FOCUSNEVER | TCS_TOOLTIPS | TCS_SINGLELINE,
        0, 0, 0, 0, Globals.hwndMain, (HMENU)IDC_TABBAR, Globals.hInstance, NULL);
    SendMessage(pane->hwndTabBar, WM_SETFONT, (WPARAM)GetStockObject(DEFAULT_GUI_FONT), MAKELPARAM(TRUE, 0));
    SetWindowSubclass(pane->hwndTabBar, _TabBarSubclassProc, 0, 0);
#ifdef D_NP3_WIN10_DARK_MODE
    if (IsDarkModeSupported() && CheckDarkModeEnabled()) {
        AllowDarkModeForWindowEx(pane->hwndTabBar, true);
        SetWindowTheme(pane->hwndTabBar, L"DarkMode_Explorer", NULL);
    }
#endif
    pane->cyTabBar = g_activePane ? g_activePane->cyTabBar : 26;

    // wrap the view's initial document as buffer 0 of this pane
    EditPane* const prevActive = g_activePane;
    g_activePane = pane; // so the tab-bar/tab macros target this pane
    sptr_t const pDoc0 = SciCall_GetDocPointer();
    SciCall_AddRefDocument(pDoc0);
    EditBuffer* const buf0 = _Tabs_NewBuffer(pDoc0);
    pane->tabs[0] = buf0;
    pane->tabCount = 1;
    pane->tabActive = 0;
    _Tabs_InsertItem(0, buf0);
    TabCtrl_SetCurSel(pane->hwndTabBar, 0);
    SciCall_SetEOLMode(Settings.DefaultEOLMode);
    Encoding_Current(Settings.DefaultEncoding);
    Style_SetDefaultLexer(pane->hwndSci);
    ShowWindow(pane->hwndTabBar, SW_SHOW);
    ShowWindow(pane->hwndSci, SW_SHOW);
    // buf0 already holds correct pristine-untitled state (from _Tabs_NewBuffer)
    g_activePane = prevActive; // caller decides who becomes active

    ++s_paneCount;
    return pane;
}

//  allocate / free split-tree nodes
static SplitNode* _Split_NewLeaf(EditPane* pane)
{
    SplitNode* const n = (SplitNode*)AllocMem(sizeof(SplitNode), HEAP_ZERO_MEMORY);
    if (n) {
        n->kind = SPL_LEAF;
        n->pane = pane;
        n->ratio = 0.5f;
...
static SplitNode* _Split_FindLeaf(SplitNode* node, const EditPane* pane)
{
    if (!node) {
        return NULL;
    }
    ...
    SplitNode* r = _Split_FindLeaf(node->child[0], pane);
    return r ? r : _Split_FindLeaf(node->child[1], pane);
}
static EditPane* _Split_FirstPane(SplitNode* node)
{
    if (!node) {
        return NULL;
    }
    ...
    EditPane* p = _Split_FirstPane(node->child[0]);
    return p ? p : _Split_FirstPane(node->child[1]);
}

//=============================================================================
//
//  Pane_DoSplit() - split the active pane into two (kind = HORZ or VERT)
//
//  Split the leaf occupied by 'existing' into (existing, new) along 'kind'.
//  newFirst => the new pane becomes child[0] (left/top), else child[1].
//  Returns the new pane (SciCall left targeting it), or NULL on failure.
static EditPane* _Pane_SplitLeaf(SplitNode* leaf, EditPane* existing, SplitKind kind, bool newFirst)
{
    if (!leaf || (s_paneCount >= NP3_MAX_PANES)) {
        return NULL;
    }
    EditPane* const np = _Pane_Create();
    if (!np) {
        return NULL;
    }
    SplitNode* const a = _Split_NewLeaf(existing); // the existing pane
    SplitNode* const b = _Split_NewLeaf(np);       // the new pane
    if (!a || !b) {
        return NULL; // (rare OOM; minor leak)
    }
    leaf->kind  = kind;
    leaf->pane  = NULL;
    leaf->ratio = 0.5f;
    leaf->child[0] = newFirst ? b : a;
    leaf->child[1] = newFirst ? a : b;
...
void Pane_DoSplit(SplitKind kind)
{
    EditPane* const src = g_activePane;
    SplitNode* const leaf = _Split_FindLeaf(g_splitRoot, src);
    if (!leaf) {
        return;
    }
    _Tabs_SaveActiveState(); // persist active state while SciCall targets src

    EditPane* const np = _Pane_SplitLeaf(leaf, src, kind, false);
    if (!np) {
        InitScintillaHandle(src->hwndSci);
        Globals.hwndEdit = src->hwndSci;
        return;
    }

    // make the new pane active (SciCall already on it)
    g_activePane = np;
    Globals.hwndEdit = np->hwndSci;
    _Pane_SyncGlobalsFromActiveBuffer(np);
...
//=============================================================================
//
//  _Pane_Destroy() - release a pane's buffers + windows, free its array slot
//
static void _Pane_Destroy(EditPane* pane)
{
    if (!pane) {
        return;
    }
    for (int i = 0; i < pane->tabCount; ++i) {
        _Tabs_FreeBuffer(pane->tabs[i]);
        pane->tabs[i] = NULL;
    }
    if (pane->hwndTabBar) {
        DestroyWindow(pane->hwndTabBar);
    }
    if (pane->hwndSci) {
        DestroyWindow(pane->hwndSci);
    }
    ZeroMemory(pane, sizeof(EditPane));
    --s_paneCount;
}
...
//=============================================================================
//
//  Pane_CloseActive() - close the active pane, collapsing the split tree.
//  Prompts to save each tab in the pane; aborts on cancel.
//
void Pane_CloseActive(void)
{
    if (s_paneCount <= 1) {
        return; // never close the last pane (use close-tab instead)
    }
    EditPane* const p = g_activePane;
    SplitNode* const leaf = _Split_FindLeaf(g_splitRoot, p);
    if (!leaf || !leaf->parent) {
        return;
    }
    // prompt-save every tab in this pane
    for (int i = 0; i < p->tabCount; ++i) {
        if (i != p->tabActive) {
            _Tabs_SaveActiveState();
            _Tabs_LoadActiveState(i);
        }
        if (!FileSave(FSF_Ask)) {
            return; // cancelled
        }
    ...
    SplitNode* const parent  = leaf->parent;
    SplitNode* const sibling = (parent->child[0] == leaf) ? parent->child[1] : parent->child[0];
    SplitNode* const gp      = parent->parent;

    // promote sibling into parent's place
    sibling->parent = gp;
    if (!gp) {
        g_splitRoot = sibling;
    } else {
    ...
    // new active pane = first leaf of the promoted subtree
    EditPane* const na = _Split_FirstPane(sibling);
    g_activePane = na;
    InitScintillaHandle(na->hwndSci);
    Globals.hwndEdit = na->hwndSci;
    _Pane_SyncGlobalsFromActiveBuffer(na);
...
//=============================================================================
//
//  Tab drag-to-dock (VS-style): drag a tab onto a pane to move it there, or
//  onto a pane edge to split-dock. A translucent hint shows the drop zone.
//
//=============================================================================

enum { DOCK_NONE = -1, DOCK_CENTER = 0, DOCK_LEFT, DOCK_RIGHT, DOCK_TOP, DOCK_BOTTOM };

static bool      s_tabDragActive = false;
static EditPane* s_tabDragPane   = NULL;
static int       s_tabDragTab    = -1;
static POINT     s_tabDragOrigin = { 0, 0 };
static EditPane* s_dockTarget    = NULL;
static int       s_dockZone      = DOCK_NONE;
static HWND      s_hwndDockHint  = NULL;

static EditPane* _Pane_FromClientPt(POINT cl)
{
    for (int i = 0; i < NP3_MAX_PANES; ++i) {
        EditPane* const p = &s_panes[i];
        if (p->hwndSci && PtInRect(&p->rc, cl)) {
            return p;
        }
    }
    return NULL;
...
static int _DockZoneOf(const RECT* rc, POINT cl)
{
    int const w = rc->right - rc->left;
    int const h = rc->bottom - rc->top;
    int const mx = w / 3;
    int const my = h / 3;
    if (cl.x < rc->left + mx)  { return DOCK_LEFT; }
    if (cl.x > rc->right - mx) { return DOCK_RIGHT; }
    if (cl.y < rc->top + my)   { return DOCK_TOP; }
    if (cl.y > rc->bottom - my){ return DOCK_BOTTOM; }
    return DOCK_CENTER;
}

static RECT _DockZoneRect(const RECT* rc, int zone)
{
    RECT z = *rc;
    int const w = rc->right - rc->left;
    int const h = rc->bottom - rc->top;
    switch (zone) {
    case DOCK_LEFT:   z.right  = rc->left + w / 2; break;
    case DOCK_RIGHT:  z.left   = rc->right - w / 2; break;
    case DOCK_TOP:    z.bottom = rc->top + h / 2; break;
    case DOCK_BOTTOM: z.top    = rc->bottom - h / 2; break;
    default: break; // DOCK_CENTER => whole pane
    }
    return z;
}

static void _DockHint_Ensure(void)
{
    if (s_hwndDockHint) {
        return;
    }
    ...
    if (!s_classReg) {
        ...
        wc.hbrBackground = CreateSolidBrush(RGB(60, 140, 230));
        wc.lpszClassName = L"NP3DockHint";
        RegisterClass(&wc);
        s_classReg = true;
    }
    s_hwndDockHint = CreateWindowEx(
        WS_EX_LAYERED | WS_EX_TRANSPARENT | WS_EX_TOOLWINDOW | WS_EX_NOACTIVATE,
        L"NP3DockHint", NULL, WS_POPUP, 0, 0, 0, 0, Globals.hwndMain, NULL, Globals.hInstance, NULL);
    if (s_hwndDockHint) {
        SetLayeredWindowAttributes(s_hwndDockHint, 0, 96, LWA_ALPHA);
    }
}

static void _DockHint_Show(RECT screenRc)
{
    _DockHint_Ensure();
    if (s_hwndDockHint) {
        SetWindowPos(s_hwndDockHint, HWND_TOPMOST, screenRc.left, screenRc.top,
                     screenRc.right - screenRc.left, screenRc.bottom - screenRc.top,
                     SWP_NOACTIVATE | SWP_SHOWWINDOW);
    }
}

static void _DockHint_Hide(void)
{
    if (s_hwndDockHint) {
        ShowWindow(s_hwndDockHint, SW_HIDE);
    }
}

//  detach tab[idx] from a pane (buffer NOT released). Switches the pane's view
//  to a neighbour if any remain; leaves it showing buf if the pane empties.
static EditBuffer* _Tabs_DetachAt(EditPane* pane, int idx)
{
    if (!pane || (idx < 0) || (idx >= pane->tabCount)) {
        return NULL;
    }
    if (pane != g_activePane) {
        SetActivePane(pane);
    }
    if (idx == pane->tabActive) {
        _Tabs_SaveActiveState(); // flush live caret/encoding into the buffer
    }
    EditBuffer* const buf = pane->tabs[idx];
    for (int i = idx; i < pane->tabCount - 1; ++i) {
        pane->tabs[i] = pane->tabs[i + 1];
    }
    pane->tabs[--pane->tabCount] = NULL;
    if (pane->hwndTabBar) {
        TabCtrl_DeleteItem(pane->hwndTabBar, idx);
    }
    if (pane->tabCount > 0) {
        int const na = (idx < pane->tabCount) ? idx : (pane->tabCount - 1);
        pane->tabActive = na;
        _Tabs_LoadActiveState(na); // view switches to neighbour (releases buf from this view)
        s_tabSwitching = true;
        TabCtrl_SetCurSel(pane->hwndTabBar, na);
        s_tabSwitching = false;
    } else {
        pane->tabActive = -1; // pane will be destroyed by the caller
    }
    return buf;
}

//  append buf to a pane and make it the active tab
static void _Tabs_AttachActivate(EditPane* pane, EditBuffer* buf)
{
    if (!pane || !buf) {
        return;
    }
    ...
    _Tabs_LoadActiveState(idx);
    s_tabSwitching = true;
    TabCtrl_SetCurSel(pane->hwndTabBar, idx);
    s_tabSwitching = false;
    _Tabs_UpdateItemText(idx);
}

//  remove an (already emptied) pane from the split tree + destroy its windows
static void _Pane_DestroyAndCollapse(EditPane* pane)
{
    SplitNode* const leaf = _Split_FindLeaf(g_splitRoot, pane);
    if (!leaf || !leaf->parent) {
        return; // cannot collapse the root
    }
    SplitNode* const parent  = leaf->parent;
    SplitNode* const sibling = (parent->child[0] == leaf) ? parent->child[1] : parent->child[0];
    SplitNode* const gp      = parent->parent;
    ...
    FreeMem(parent);
    FreeMem(leaf);
    _Pane_Destroy(pane); // tabCount==0 => frees no buffers
}

//  perform a drag-drop: move srcPane.tab[srcIdx] onto dstPane in 'zone'
static void _Tabs_DockDrop(EditPane* srcPane, int srcIdx, EditPane* dstPane, int zone)
{
    if (!srcPane || (srcIdx < 0) || !dstPane || (zone == DOCK_NONE)) {
        return;
    }
    if ((zone == DOCK_CENTER) && (dstPane == srcPane)) {
        return; // dropped onto its own pane center => nothing to do
    }
    bool const srcWasOnly = (srcPane->tabCount <= 1);
    if ((zone != DOCK_CENTER) && (dstPane == srcPane) && srcWasOnly) {
        return; // splitting a pane off its only tab => no-op
    }

    EditBuffer* const buf = _Tabs_DetachAt(srcPane, srcIdx);
    if (!buf) {
        return;
    }

    EditPane* target = NULL;
    if (zone == DOCK_CENTER) {
        _Tabs_AttachActivate(dstPane, buf);
        target = dstPane;
    } else {
        SplitNode* const leaf = _Split_FindLeaf(g_splitRoot, dstPane);
        SplitKind const kind = (zone == DOCK_LEFT || zone == DOCK_RIGHT) ? SPL_VERT : SPL_HORZ;
        bool const newFirst  = (zone == DOCK_LEFT || zone == DOCK_TOP);
        _Tabs_SaveActiveState();
        EditPane* const np = leaf ? _Pane_SplitLeaf(leaf, dstPane, kind, newFirst) : NULL;
        if (np) {
            _Tabs_AttachActivate(np, buf);
            // drop the new pane's throwaway pristine "Untitled" buffer
            if ((np->tabCount == 2) && Path_IsEmpty(np->tabs[0]->path)) {
                _Tabs_FreeBuffer(np->tabs[0]);
                np->tabs[0] = np->tabs[1];
                np->tabs[1] = NULL;
                np->tabCount = 1;
                np->tabActive = 0;
                if (np->hwndTabBar) {
                    TabCtrl_DeleteItem(np->hwndTabBar, 0);
                    s_tabSwitching = true;
                    TabCtrl_SetCurSel(np->hwndTabBar, 0);
                    s_tabSwitching = false;
                }
            }
            target = np;
        } else {
            _Tabs_AttachActivate(dstPane, buf); // fallback: just move into dst
            target = dstPane;
        }
    }

    if ((srcPane->tabCount == 0) && (srcPane != target)) {
        _Pane_DestroyAndCollapse(srcPane);
    }
...
//=============================================================================
//
//  SplitLayout_Resize() - recursively position panes + splitter bars
//
static void SplitLayout_Resize(SplitNode* node, RECT rc)
{
    if (!node) {
    ...
    if (node->kind == SPL_LEAF) {
        EditPane* const pane = node->pane;
        if (!pane) {
            return;
        }
        pane->rc = rc;
        int const cyTab = pane->cyTabBar;
        SetWindowPos(pane->hwndTabBar, NULL, rc.left, rc.top, rc.right - rc.left, cyTab,
                     SWP_NOZORDER | SWP_NOACTIVATE);
        SetWindowPos(pane->hwndSci, NULL, rc.left, rc.top + cyTab,
                     rc.right - rc.left, max_i(0, (rc.bottom - rc.top) - cyTab),
                     SWP_NOZORDER | SWP_NOACTIVATE);
        return;
    }

    RECT r0 = rc, r1 = rc;
    if (node->kind == SPL_VERT) { // side-by-side
        int const total = rc.right - rc.left - NP3_SPLIT_BAR;
        int const w0 = (int)(total * node->ratio + 0.5f);
        r0.right = rc.left + w0;
        r1.left  = r0.right + NP3_SPLIT_BAR;
        node->bar.left = r0.right; node->bar.right = r1.left;
        node->bar.top = rc.top;   node->bar.bottom = rc.bottom;
    } else { // SPL_HORZ: stacked
        int const total = rc.bottom - rc.top - NP3_SPLIT_BAR;
        int const h0 = (int)(total * node->ratio + 0.5f);
        r0.bottom = rc.top + h0;
        r1.top    = r0.bottom + NP3_SPLIT_BAR;
        node->bar.top = r0.bottom; node->bar.bottom = r1.top;
        node->bar.left = rc.left;  node->bar.right = rc.right;
    }
    SplitLayout_Resize(node->child[0], r0);
    SplitLayout_Resize(node->child[1], r1);
}

//  splitter hit-testing / dragging (called from MainWndProc mouse handlers)
static SplitNode* _Split_HitBar(SplitNode* node, POINT pt)
{
    if (!node || node->kind == SPL_LEAF) {
        return NULL;
    }
    if (PtInRect(&node->bar, pt)) {
        return node;
    }
    SplitNode* r = _Split_HitBar(node->child[0], pt);
    return r ? r : _Split_HitBar(node->child[1], pt);
}

bool Split_OnLButtonDown(HWND hwnd, POINT pt)
{
    SplitNode* const bar = _Split_HitBar(g_splitRoot, pt);
    if (!bar) {
        return false;
    }
    s_dragBar = bar;
...
void Split_OnMouseMove(HWND hwnd, POINT pt)
{
    ...
    // find the rect covered by the dragged node = union of its children rects
    SplitNode* const n = s_dragBar;
    if (n->kind == SPL_VERT) {
        int const left  = (n->child[0]->kind == SPL_LEAF) ? n->child[0]->pane->rc.left  : n->bar.left;
        int const right = (n->child[1]->kind == SPL_LEAF) ? n->child[1]->pane->rc.right : n->bar.right;
        int const totalw = right - left - NP3_SPLIT_BAR;
        if (totalw > 20) {
            float const r = (float)(pt.x - left) / (float)totalw;
            n->ratio = (r < 0.1f) ? 0.1f : ((r > 0.9f) ? 0.9f : r);
        }
    } else {
        int const top    = (n->child[0]->kind == SPL_LEAF) ? n->child[0]->pane->rc.top    : n->bar.top;
        int const bottom = (n->child[1]->kind == SPL_LEAF) ? n->child[1]->pane->rc.bottom : n->bar.bottom;
        int const totalh = bottom - top - NP3_SPLIT_BAR;
        if (totalh > 20) {
            float const r = (float)(pt.y - top) / (float)totalh;
            n->ratio = (r < 0.1f) ? 0.1f : ((r > 0.9f) ? 0.9f : r);
        }
    }
    UpdateUI(hwnd);
...
bool Split_OnSetCursor(HWND hwnd)
{
    ...
    SplitNode* const bar = _Split_HitBar(g_splitRoot, pt);
    if (bar) {
        SetCursor(LoadCursor(NULL, (bar->kind == SPL_VERT) ? IDC_SIZEWE : IDC_SIZENS));
        return true;
    }
    return false;
}
...
//=============================================================================
//
//  _Tabs_MoveTab() - reorder a tab within its pane (drag-reorder)
//
static void _Tabs_MoveTab(EditPane* pane, int from, int to)
{
    if (pane != g_activePane) {
        SetActivePane(pane);
    }
    if ((from < 0) || (to < 0) || (from >= s_tabCount) || (to >= s_tabCount) || (from == to)) {
        return;
    }
    EditBuffer* const moved = s_tabs[from];
    if (from < to) {
        for (int i = from; i < to; ++i) { s_tabs[i] = s_tabs[i + 1]; }
    } else {
        for (int i = from; i > to; --i) { s_tabs[i] = s_tabs[i - 1]; }
    }
    s_tabs[to] = moved;
    s_tabActive = to; // the dragged tab was the selected one
    for (int i = 0; i < s_tabCount; ++i) {
        _Tabs_UpdateItemText(i);
    }
    s_tabSwitching = true;
    TabCtrl_SetCurSel(s_hwndTabBar, to);
    s_tabSwitching = false;
}

//=============================================================================
//
//  _TabBarSubclassProc() - per-pane tab-strip subclass:
//  middle-click closes a tab; left-drag reorders within a pane OR drags the
//  tab onto another pane / pane edge to move/split-dock it (VS-style).
//
static LRESULT CALLBACK _TabBarSubclassProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam,
                                            UINT_PTR uId, DWORD_PTR dwRef)
{
    UNREFERENCED_PARAMETER(uId);
    UNREFERENCED_PARAMETER(dwRef);
    switch (msg) {
    case WM_MBUTTONUP: {
        TCHITTESTINFO hti = { 0 };
        hti.pt.x = GET_X_LPARAM(lParam);
        hti.pt.y = GET_Y_LPARAM(lParam);
        int const idx = TabCtrl_HitTest(hwnd, &hti);
        EditPane* const p = _Pane_FindByTabBar(hwnd);
        if ((idx >= 0) && p) {
            if (p != g_activePane) {
                SetActivePane(p);
            }
    ...
    case WM_LBUTTONDBLCLK: {
        // double-click empty strip area => open a new tab
        TCHITTESTINFO hti = { 0 };
        hti.pt.x = GET_X_LPARAM(lParam);
        hti.pt.y = GET_Y_LPARAM(lParam);
        EditPane* const p = _Pane_FindByTabBar(hwnd);
        if ((TabCtrl_HitTest(hwnd, &hti) < 0) && p) {
            if (p != g_activePane) {
                SetActivePane(p);
            }
            SendWMCommand(Globals.hwndMain, IDM_FILE_NEW);
        }
        return 0;
    }
    case WM_LBUTTONDOWN: {
        ...
        if ((idx >= 0) && p) {
            ...
            s_tabDragPane = p;
            s_tabDragTab = idx;
            s_tabDragActive = false;
            GetCursorPos(&s_tabDragOrigin);
            SetCapture(hwnd);
            return 0; // we fully own the click (selection already applied)
        }
        break;
    }
    case WM_MOUSEMOVE: {
        if (s_tabDragPane && (GetCapture() == hwnd)) {
            POINT scr; GetCursorPos(&scr);
            if (!s_tabDragActive &&
                ((abs(scr.x - s_tabDragOrigin.x) > 6) || (abs(scr.y - s_tabDragOrigin.y) > 6))) {
                s_tabDragActive = true;
            }
            if (s_tabDragActive) {
                POINT cl = scr; ScreenToClient(Globals.hwndMain, &cl);
                EditPane* const tp = _Pane_FromClientPt(cl);
                if (tp) {
                    s_dockTarget = tp;
                    s_dockZone = _DockZoneOf(&tp->rc, cl);
                    RECT zr = _DockZoneRect(&tp->rc, s_dockZone);
                    POINT a = { zr.left, zr.top };
                    POINT b = { zr.right, zr.bottom };
                    ClientToScreen(Globals.hwndMain, &a);
                    ClientToScreen(Globals.hwndMain, &b);
                    RECT sr = { a.x, a.y, b.x, b.y };
                    _DockHint_Show(sr);
                } else {
                    s_dockTarget = NULL;
                    s_dockZone = DOCK_NONE;
                    _DockHint_Hide();
                }
                SetCursor(LoadCursor(NULL, IDC_SIZEALL));
            }
    ...
    case WM_LBUTTONUP: {
        if (s_tabDragPane && (GetCapture() == hwnd)) {
            ReleaseCapture();
            _DockHint_Hide();
            EditPane* const sp = s_tabDragPane;
            int const si = s_tabDragTab;
            bool const dragged = s_tabDragActive;
            EditPane* const dt = s_dockTarget;
            int const dz = s_dockZone;
            s_tabDragPane = NULL; s_tabDragTab = -1; s_tabDragActive = false;
            s_dockTarget = NULL; s_dockZone = DOCK_NONE;
            if (dragged && dt) {
                if ((dt == sp) && (dz == DOCK_CENTER)) {
                    // reorder within the same pane if released over a tab
                    POINT cl; GetCursorPos(&cl); ScreenToClient(hwnd, &cl);
                    TCHITTESTINFO hti = { 0 }; hti.pt = cl;
                    int const drop = TabCtrl_HitTest(hwnd, &hti);
                    if ((drop >= 0) && (drop != si)) {
                        _Tabs_MoveTab(sp, si, drop);
                    }
                } else {
                    _Tabs_DockDrop(sp, si, dt, dz);
                }
            }
            return 0;
        }
...
//  re-apply dark/light theming to a pane's tab strip (called on theme change)
static void _TabBar_ApplyTheme(HWND hwndBar)
{
#ifdef D_NP3_WIN10_DARK_MODE
    bool const dark = IsDarkModeSupported() && CheckDarkModeEnabled();
    AllowDarkModeForWindowEx(hwndBar, dark);
    SetWindowTheme(hwndBar, dark ? L"DarkMode_Explorer" : NULL, NULL);
#else
    UNREFERENCED_PARAMETER(hwndBar);
#endif
}

//  on theme change: restyle every pane's view + re-theme every tab strip
static void _Panes_OnThemeChanged(void)
{
    EditPane* const orig = g_activePane;
    for (int i = 0; i < NP3_MAX_PANES; ++i) {
        EditPane* const p = &s_panes[i];
        if (!p->hwndSci) {
            continue;
        ...
        if (p != orig) {
            SetActivePane(p);
            Style_ResetCurrentLexer(p->hwndSci);
        }
        _TabBar_ApplyTheme(p->hwndTabBar);
    }
    if (g_activePane != orig) {
        SetActivePane(orig);
    }
}
...
//=============================================================================
//
//  _InputBox() - minimal modal text prompt built from an in-memory dialog
//  template (no .rc resource needed, so it works with the language DLLs).
//
...
static INT_PTR CALLBACK _InputBoxProc(HWND hdlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    static INPUTBOXDATA_T* s_d = NULL;
    switch (msg) {
    case WM_INITDIALOG:
        s_d = (INPUTBOXDATA_T*)lParam;
        SetWindowText(hdlg, s_d->title);
        SetDlgItemText(hdlg, 101, s_d->prompt);
        SetDlgItemText(hdlg, 102, s_d->io);
        SendDlgItemMessage(hdlg, 102, EM_SETSEL, 0, -1);
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            if (s_d) {
                GetDlgItemText(hdlg, 102, s_d->io, s_d->cch);
            }
            EndDialog(hdlg, IDOK);
            return TRUE;
        case IDCANCEL:
...
// append a (DWORD-aligned) WORD stream helper
static WORD* _DlgAlignDword(WORD* p)
{
    ULONG_PTR ul = (ULONG_PTR)p;
    ul = (ul + 3) & ~(ULONG_PTR)3;
    return (WORD*)ul;
}
static WORD* _DlgAddStr(WORD* p, LPCWSTR s)
{
    if (s) {
        while (*s) { *p++ = (WORD)*s++; }
    }
...
static WORD* _DlgAddItem(WORD* p, DWORD style, short x, short y, short cx, short cy,
                         WORD id, WORD ctlClassAtom, LPCWSTR text)
{
    p = _DlgAlignDword(p);
    *(DWORD*)p = style;            p += 2;
    ...
    *(short*)p = cy;               p += 1;
    *(WORD*)p = id;                p += 1;
    *p++ = 0xFFFF; *p++ = ctlClassAtom;     // class as atom
    p = _DlgAddStr(p, text);
    *p++ = 0;                      // no creation data
    return p;
}

static bool _InputBox(HWND parent, LPCWSTR title, LPCWSTR prompt, LPWSTR io, int cch)
{
    static WORD s_tmpl[512];
    WORD* p = s_tmpl;
    DLGTEMPLATE* dt = (DLGTEMPLATE*)p;
    dt->style = DS_MODALFRAME | DS_SETFONT | DS_CENTER | WS_POPUP | WS_CAPTION | WS_SYSMENU;
    dt->dwExtendedStyle = 0;
    dt->cdit = 4;
    dt->x = 0; dt->y = 0; dt->cx = 240; dt->cy = 78;
    p += sizeof(DLGTEMPLATE) / sizeof(WORD);
    *p++ = 0;            // no menu
    *p++ = 0;            // default class
    p = _DlgAddStr(p, title);
    *p++ = 9;            // font size
    p = _DlgAddStr(p, L"Segoe UI");

    // static prompt, edit, OK, Cancel
    p = _DlgAddItem(p, WS_CHILD | WS_VISIBLE | SS_LEFT, 8, 7, 224, 10, 101, 0x0082, prompt);    // STATIC
    p = _DlgAddItem(p, WS_CHILD | WS_VISIBLE | WS_BORDER | WS_TABSTOP | ES_AUTOHSCROLL, 8, 20, 224, 13, 102, 0x0081, L""); // EDIT
    p = _DlgAddItem(p, WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_DEFPUSHBUTTON, 128, 58, 50, 14, IDOK, 0x0080, L"OK");       // BUTTON
    p = _DlgAddItem(p, WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_PUSHBUTTON, 182, 58, 50, 14, IDCANCEL, 0x0080, L"Cancel");  // BUTTON

    INPUTBOXDATA_T d = { title, prompt, io, cch };
    INT_PTR const r = DialogBoxIndirectParamW(Globals.hInstance, dt, parent, _InputBoxProc, (LPARAM)&d);
    return (r == IDOK);
}

//=============================================================================
//
//  Tabs_RenameActive() - give the active editor a custom display name (works
//  for untitled buffers too; persisted across sessions via the title excerpt).
//
void Tabs_RenameActive(void)
{
    if (!Tabs_IsActive()) {
        return;
    }
    WCHAR name[MIDSZ_BUFFER] = { L'\0' };
    if (StrIsNotEmpty(s_wchTitleExcerpt)) {
        StringCchCopyW(name, COUNTOF(name), s_wchTitleExcerpt);
...
//=============================================================================
//
//  Tabs_WipeAll() - close every editor and erase all remembered session data.
//
static void _Session_Wipe(void)
{
    HPATHL hIni = Path_Allocate(NULL);
    if (_Session_IniPath(false, hIni) && Path_IsExistingFile(hIni)) {
        Path_DeleteFile(hIni);
    }
    Path_Release(hIni);
    for (int i = 1; i <= NP3_MAX_TABS; ++i) {
        WCHAR nm[32];
        StringCchPrintfW(nm, COUNTOF(nm), L"tab%d.bak", i);
        HPATHL hb = _Session_BakPath(nm);
        if (Path_IsExistingFile(hb)) {
            Path_DeleteFile(hb);
        }
        Path_Release(hb);
    }
...
void Tabs_WipeAll(void)
{
    ...
    // 1) reduce to a single pane (destroy all the others; no save prompt)
    EditPane* const keep = g_activePane;
    for (int i = 0; i < NP3_MAX_PANES; ++i) {
        EditPane* const p = &s_panes[i];
        if (p->hwndSci && (p != keep)) {
            _Pane_DestroyAndCollapse(p);
        }
    }
    SetActivePane(keep);

    // 2) reset the surviving pane to one fresh blank untitled tab
    sptr_t const newDoc = SciCall_CreateDocument(0, SC_DOCUMENTOPTION_DEFAULT);
    SciCall_SetDocPointer(newDoc);                 // view shows the new doc; old one released
    for (int t = 0; t < keep->tabCount; ++t) {
        _Tabs_FreeBuffer(keep->tabs[t]);
        keep->tabs[t] = NULL;
    }
    if (keep->hwndTabBar) {
        TabCtrl_DeleteAllItems(keep->hwndTabBar);
    }
    EditBuffer* const b0 = _Tabs_NewBuffer(newDoc);
    keep->tabs[0] = b0;
    keep->tabCount = 1;
    keep->tabActive = 0;
    _Tabs_InsertItem(0, b0);
    if (keep->hwndTabBar) {
        TabCtrl_SetCurSel(keep->hwndTabBar, 0);
    }
    Path_Empty(Paths.CurrentFile, false);
    StringCchCopyW(s_wchTitleExcerpt, COUNTOF(s_wchTitleExcerpt), L"");
    EditSetNewText(keep->hwndSci, "", 0, true, false);
    Style_SetDefaultLexer(keep->hwndSci);
    SetSaveDone();

    // 3) erase remembered session data on disk
    _Session_Wipe();
    ...
    UpdateTitlebar(Globals.hwndMain);
    Tabs_SyncActiveFromGlobals();
    SetFocus(keep->hwndSci);
}

//=============================================================================
//
//  Session split-layout (de)serialization: rebuild the pane tree on restore
//
typedef struct PNODE_T {
    int   kind;                       // SPL_LEAF / SPL_HORZ / SPL_VERT
    float ratio;
    int   leaf[NP3_MAX_TABS];
    int   leafCount;
    int   activeFlat;
    struct PNODE_T* c0;
    struct PNODE_T* c1;
...
static PNODE_T s_pnodePool[NP3_MAX_PANES * 2 + 4];
static int     s_pnodeUsed = 0;

static PNODE_T* _PNodeAlloc(void)
{
    if (s_pnodeUsed >= (int)COUNTOF(s_pnodePool)) {
        return NULL;
    }
    PNODE_T* const n = &s_pnodePool[s_pnodeUsed++];
    ZeroMemory(n, sizeof(*n));
    return n;
}

//  recursive-descent parser:  node := 'L'<active>':'<i,...>  |  ('V'|'H')<pct>'('node')('node')'
static PNODE_T* _LayoutParse(const WCHAR** pp)
{
    const WCHAR* p = *pp;
    PNODE_T* const node = _PNodeAlloc();
    if (!node) {
        return NULL;
    }
    if (*p == L'L') {
        ...
        WCHAR* end = NULL;
        node->kind = SPL_LEAF;
        node->activeFlat = (int)wcstol(p, &end, 10);
        p = end;
        if (*p == L':') {
            ++p;
        }
        while (*p && (*p != L')')) {
            int const v = (int)wcstol(p, &end, 10);
            p = end;
            if (node->leafCount < NP3_MAX_TABS) {
                node->leaf[node->leafCount++] = v;
            }
            if (*p == L',') { ++p; } else { break; }
        }
    }
    else if ((*p == L'V') || (*p == L'H')) {
        node->kind = (*p == L'V') ? SPL_VERT : SPL_HORZ;
        ++p;
        WCHAR* end = NULL;
        int const pct = (int)wcstol(p, &end, 10);
        p = end;
        node->ratio = (float)pct / 100.0f;
        if (node->ratio < 0.1f) { node->ratio = 0.1f; }
        if (node->ratio > 0.9f) { node->ratio = 0.9f; }
        if (*p == L'(') { ++p; }
        node->c0 = _LayoutParse(&p);
        if (*p == L')') { ++p; }
        if (*p == L'(') { ++p; }
        node->c1 = _LayoutParse(&p);
        if (*p == L')') { ++p; }
    }
    else {
        *pp = p;
...
static void _LayoutRealize(PNODE_T* node, SplitNode* leafNode, EditPane* pane)
{
    if (!node || !pane) {
        return;
    }
    if (node->kind == SPL_LEAF) {
        SetActivePane(pane);
        s_restoreFirstPending = true; // first entry reuses this pane's pristine tab
        for (int k = 0; k < node->leafCount; ++k) {
            _RestoreOneByFlat(node->leaf[k] - 1);
        }
        for (int t = 0; t < pane->tabCount; ++t) {
            if (pane->tabs[t]->saveIdx == node->activeFlat) {
                Tabs_Activate(t);
                break;
            }
        }
    ...
    EditPane* const np = _Pane_SplitLeaf(leafNode, pane, (SplitKind)node->kind, false);
    if (!np) {
        _LayoutRealize(node->c0, leafNode, pane);
        _LayoutRealize(node->c1, leafNode, pane);
        return;
    }
    leafNode->ratio = node->ratio;
    _LayoutRealize(node->c0, leafNode->child[0], pane);
    _LayoutRealize(node->c1, leafNode->child[1], np);
}

static bool _Layout_Realize(LPCWSTR layout)
{
    s_pnodeUsed = 0;
    const WCHAR* p = layout;
    PNODE_T* const root = _LayoutParse(&p);
    if (!root) {
        return false;
    }
    _LayoutRealize(root, g_splitRoot, g_activePane);

    // drop any leftover pristine tabs (saveIdx==0) the split machinery created
    // but that no restored buffer reused, keeping >=1 tab per pane
    for (int pi = 0; pi < NP3_MAX_PANES; ++pi) {
        EditPane* const pane = &s_panes[pi];
        if (!pane->hwndSci) {
            continue;
        }
        SetActivePane(pane);
        int t = 0;
        while ((pane->tabCount > 1) && (t < pane->tabCount)) {
            if ((t != pane->tabActive) && (pane->tabs[t]->saveIdx == 0)) {
                _Tabs_RemoveAt(t); // tab t is not the shown one -> safe to drop
            } else {
                ++t;
            }
        }
...
LRESULT MsgCreate(HWND hwnd, WPARAM wParam,LPARAM lParam)
{
    ...
    // Multi-document tab bar (wraps the initial document as the first tab)
    Tabs_CreateBar(hwnd, hInstance);
...
LRESULT MsgEndSession(HWND hwnd, UINT umsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (!bShutdownOK) {
        ...
        // persist the open-tabs session, then save the rest of the settings
        Tabs_SaveSession();
...
LRESULT MsgThemeChanged(HWND hwnd, WPARAM wParam,LPARAM lParam)
{
    ...
    if (Globals.hwndMain) {
        ...
        // restyle the other split-pane views + re-theme all tab strips
        _Panes_OnThemeChanged();
...
LRESULT MsgSize(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    // Lay out the split-pane tree (each pane = tab strip + Scintilla view).
    // Falls back to the legacy single-view positioning if panes aren't up yet.
    if (g_splitRoot) {
        RECT rcEdit = { x, y, x + cx, y + cy };
        SplitLayout_Resize(g_splitRoot, rcEdit);
    } else {
...
static LRESULT _OnDropOneFile(HWND hwnd, HPATHL hFilePath, WININFO* wi)
{
    ...
    if (Path_IsExistingDirectory(hFilePath)) {
        if (OpenFileDlg(Globals.hwndMain, hFilePath, hFilePath)) {
            FileLoadTab(hFilePath, fLoadFlags, 0, 0);
        }
    }
...
static bool _HandleFileCommands(HWND hwnd, UINT umsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case IDM_FILE_NEW: {
        ...
        if (hsel && !isDir) {
            ...
            if (FileLoadTab(hsel, fLoad, 0, 0) && lineNum > 0) {
            ...
            if (OpenFileDlg(Globals.hwndMain, hsel, hsel)) {
                FileLoadTab(hsel, fLoad, 0, 0);
            }
        }
        else {
            // blank "New": always open a fresh blank tab (allow many empty tabs)
            if (Tabs_IsActive()) {
                Tabs_AddAndActivate();
            }
            HPATHL hempty = Path_Allocate(L"");
            FileLoad(hempty, fLoadFlags | (Tabs_IsActive() ? FLF_DontSave : 0), 0, 0);
            Path_Release(hempty);
            Tabs_SyncActiveFromGlobals();
        }
    ...
    case IDM_FILE_OPEN: {
        ...
        fLoadFlags |= Settings.SkipANSICodePageDetection ? FLF_SkipANSICPDetection : 0;
        if (Tabs_IsActive()) {
            // resolve the path ourselves so the file lands in its own tab
            HPATHL hdir = Path_Copy(Paths.CurrentFile);
            Path_RemoveFileSpec(hdir);
            if (OpenFileDlg(Globals.hwndMain, hfile_pth, Path_IsNotEmpty(hdir) ? hdir : NULL)) {
                FileLoadTab(hfile_pth, fLoadFlags, 0, 0);
            }
            Path_Release(hdir);
        }
        else {
    ...
    case IDM_FILE_CLOSETAB:
        if (Tabs_IsActive()) {
            Tabs_CloseAt(Tabs_ActiveIndex());
        }
        break;

    case IDM_FILE_CLOSEOTHERTABS:
        if (Tabs_IsActive()) {
            int keepId = -1;
            if (Tabs_ActiveIndex() >= 0) {
                keepId = s_tabs[Tabs_ActiveIndex()]->id;
            }
            // close everything except the buffer with keepId
            for (int i = Tabs_Count() - 1; i >= 0; --i) {
                if (s_tabs[i]->id != keepId) {
                    if (!Tabs_CloseAt(i)) {
    ...
    case IDM_VIEW_SPLIT_VERT:   // side-by-side
        Pane_DoSplit(SPL_VERT);
        break;

    case IDM_VIEW_SPLIT_HORZ:   // stacked
        Pane_DoSplit(SPL_HORZ);
        break;

    case IDM_VIEW_CLOSEPANE:
        Pane_CloseActive();
        break;

    case IDM_FILE_RENAMETAB:
        Tabs_RenameActive();
        break;
...
LRESULT MsgNotify(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    case IDC_EDIT: {
        // route per active pane; switch active pane when another view is focused
        if (pnmh->code == SCN_FOCUSIN) {
            EditPane* const p = _Pane_FindBySci(pnmh->hwndFrom);
            if (p && (p != g_activePane)) {
                SetActivePane(p);
            }
        }
        if (pnmh->hwndFrom == g_activePane->hwndSci) {
            bool bModified = false;
    ...
    case IDC_TABBAR:
        ...
        case TCN_SELCHANGE:
            if (!s_tabSwitching) {
                EditPane* const p = _Pane_FindByTabBar(pnmh->hwndFrom);
                if (p) {
                    if (p != g_activePane) {
                        SetActivePane(p);
                    }
                    Tabs_Activate(TabCtrl_GetCurSel(p->hwndTabBar));
                }
            }
            result = TRUE;
            break;
        case NM_RCLICK: {
            // right-click on a tab strip => context menu (tab + split actions)
            EditPane* const pclk = _Pane_FindByTabBar(pnmh->hwndFrom);
            if (pclk && (pclk != g_activePane)) {
                SetActivePane(pclk);
            }
            POINT pt; GetCursorPos(&pt);
            POINT ptc = pt; ScreenToClient(s_hwndTabBar, &ptc);
            TCHITTESTINFO hti = { 0 }; hti.pt = ptc;
            int const idx = TabCtrl_HitTest(s_hwndTabBar, &hti);
            if (idx >= 0) {
                if (idx != s_tabActive) {
                    Tabs_Activate(idx);
                }
...
bool FileLoad(const HPATHL hfile_pth, const FileLoadFlags fLoadFlags, const DocPos curPos, const DocLn visLn)
{
    ...
    if (fSuccess) {
        Tabs_SyncActiveFromGlobals(); // keep the active tab caption/path in sync
    }
    return fSuccess;
}
